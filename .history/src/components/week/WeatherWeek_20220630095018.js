...
const WeatherWeek = () => {
  ...
  const { data, dataDetails, isLoading } = useSelector(
    (state) => state.weather
  );
  ...
  return (
    <>
      {!isLoading && (
        <>
          <WeatherWeekSkeleton></WeatherWeekSkeleton>
          <WeatherWeekSkeleton></WeatherWeekSkeleton>
          <WeatherWeekSkeleton></WeatherWeekSkeleton>
          <WeatherWeekSkeleton></WeatherWeekSkeleton>
        </>
...
const WeatherWeekSkeleton = ({ dataWeek, onClick = () => {} }) => {
  const { dataDetails } = useSelector((state) => state.weather);
  return (
    <WeatherWeekStyles>
      {dataWeek &&
        dataWeek.length > 0 &&
